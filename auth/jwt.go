...
//go:generate go run github.com/matryer/moq -out mog_test.go . Store
...
func (j *JWTer) GenerateToken(ctx context.Context, u entity.User) ([]byte, error) {
	tok, err := jwt.NewBuilder().
		...
		Issuer(`github.com/ucho456/go_todo_app`).
		...
		Build()
	if err != nil {
		return nil, fmt.Errorf("GetToken: falied to build token: %w", err)
	}
